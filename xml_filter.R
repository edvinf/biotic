...
test_drop_childless_stations <- function(){
  tree <- xmlParse(test_refl_2015)
  retain_species_catch(tree, c("171342", "172930"))
  xpath = "//m:fishstation[not(m:catchsample)]"
  xpath2 = "//m:fishstation"
  namespaces="m"
  print(length(getNodeSet(tree, xpath2, namespaces)))
  print(length(getNodeSet(tree, xpath, namespaces)))
  drop_childless_stations(tree)
  print(length(getNodeSet(tree, xpath2, namespaces)))
  print(length(getNodeSet(tree, xpath, namespaces)))
}

test_retain_species_catch <- function(){
  tree <- xmlParse(test_refl_2015)
  ...
  print(length(getNodeSet(tree, xpath, namespaces)))
  print(length(getNodeSet(tree, xpath2, namespaces)))
  print(length(getNodeSet(tree, xpath3, namespaces)))
  retain_species_catch(tree, c("171342", "172930"))
...
test_apply_filters <- function(){
  apply_xml_filters(test_refl_2015, "/dev/stdout", c(function(tree){retain_species_catch(tree, c("171342", "172930"))}, drop_childless_stations, drop_childless_missions))
}